...
<!--
Ugly Code, replace with loop or figure out mapply, time permitting. Difficult if column size were much greater.
-->
...
crossTable$expectedScore<- round ( rowSums ( select(crossTable, matches("^R\\dEx")) , na.rm = TRUE) , digits = 2)

crossTable <- crossTable %>% mutate( scoreDiff = TotalPts - expectedScore )

reactable( select( crossTable, "Name" , "TotalPts" , "expectedScore", "scoreDiff")[order(crossTable$scoreDiff, decreasing = TRUE),])
```

...
```
### Tidy Data
I will now use the `pivot_longer`,`separate`, and `pivot_wider` functions to put the data in a tidy format. Additionally, for the purposes of this assignment I will only consider schools in New York State, I will use the `filter` function to only show these. I will also discard columns that are not needed. 
```{r dataset-1-tidy}
retentionRatesNY <- 
  retentionRates %>% 
...
datatable(retentionRatesNY)
```
### Analyze Data

I will first plot the distribution of full time retention rates for New York state, the distribution is left skewed bounded by 100 on the top. It appears to be almost normally distributed. 
```{r dataset-1-analysis-histogram }
retentionRatesNY2016 <- retentionRatesNY %>% 
  filter(year == 2016)
ggplot(retentionRatesNY2016 , aes(x=`Full-time retention rate`)) + geom_histogram(binwidth = 10, na.rm = T) + geom_vline(xintercept =  mean(retentionRatesNY2016$`Full-time retention rate`,na.rm = T) , col = "red", size = 1.5)
```

I will now consider all CUNY Schools by filtering the data set further. 
```{r dataset-1-CUNY }
retentionRatesNY2016CUNY <- retentionRatesNY2016 %>% 
  filter(grepl('*CUNY*',`Institution Name`))
datatable( retentionRatesNY2016CUNY)
```
From this I will now use the `mutate` function on the data to see how far each CUNY School deviates from the mean score for NY State.
```{r dataset-1-CUNY-zscore}
sd.nySchools <- sd(retentionRatesNY2016$`Full-time retention rate`,na.rm = T)
mean.nySchools <- mean(retentionRatesNY2016$`Full-time retention rate`, na.rm = T)
retentionRatesNY2016CUNY <- retentionRatesNY2016CUNY %>% 
  mutate( Z.ScoreFullTime = round( ( (`Full-time retention rate` - mean.nySchools )/sd.nySchools)  ,2 ) )  %>% 
  select(`Institution Name`, `Z.ScoreFullTime`) %>% 
...
### Conclusion
The colleges at the top of the list have the highest retention in comparison to their counterparts at the bottom, when considering all schools in New York State. It appears that community colleges have lower retention rates than the senior colleges. 
...
```
### Conclusion
From the data it would appear that Equatorial Guinea has had the highest increase in GDP from 1960-2019. 
